...
const { atomicWrite, safeRead, STATE_DIR } = require('./quality-state');

const COVERAGE_MAP_FILE = path.join(STATE_DIR, 'coverage-map.json');
...
function loadCoverageMap() {
  const map = safeRead(COVERAGE_MAP_FILE);
...
function saveCoverageMap(map) {
  ...
  atomicWrite(COVERAGE_MAP_FILE, map);
  return map;
}
