 ...
 * @type {string}
 */
const IRON_LOOP_MARKER = '## Execution Steps (Iron Loop 7-15)';
...
function validateForTodo(planPath) {
  if (!hasIronLoopSteps(planPath)) {
    return {
      valid: false,
      error: 'Plan missing Iron Loop steps 7-15. Generate them first.'
    };
  }
  return { valid: true };
}
 ...
 * @returns {string} Markdown string with steps 7-15 template
 */
