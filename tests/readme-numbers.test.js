...
describe('Ground truth — project counts (sanity checks)', () => {
  ...
  it('src/lib/: 113 JS modules at top level (task-view for NB2 + notes.js for the NOTES surface)', () => {
    assert.equal(countTopLevelJs('src/lib'), 113);
  });
...
describe('README — explicit numeric claims match reality', () => {
  ...
  it('Project structure: 113 JS modules in src/lib', () => {
    assert.match(README, /113 JS modules/);
  });
